...
class Campaign_Manager_Recurring {
    ...
    public function save_recurring() {
        ...
        // Adiciona a opção de exclusão ao JSON de providers_config
    ...
    public function execute_recurring_now() {
        ...
        // Recebe opção de exclusão da execução (pode sobrescrever a config salva)
    ...
    private function execute_campaign($campaign) {
        ...
        try {
            ...
            // Verifica se a exclusão de telefones recentes está ativada
            $exclude_recent_phones = isset($providers_config['exclude_recent_phones']) ? intval($providers_config['exclude_recent_phones']) : 1; // Default: ativado
            
            // 2. 🎯 Busca registros (com ou sem exclusão de telefones recentes)
            if ($exclude_recent_phones) {
                // Busca excluindo telefones que já receberam mensagem recentemente
                ...
                $records = Campaign_Manager_Filters::get_filtered_records(
                    $campaign['tabela_origem'],
                    $filters,
                    $campaign['record_limit']
                );
            ...
            foreach ($distribution as $provider => $provider_records) {
                ...
                foreach ($provider_records as $record) {
                    ...
                    // Nota: Se a busca já excluiu telefones recentes, esta verificação é redundante, mas serve como segurança extra
